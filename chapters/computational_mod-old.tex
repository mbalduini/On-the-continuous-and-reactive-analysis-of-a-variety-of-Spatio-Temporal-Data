...
\textcolor{red}{Those reflections inspired the idea to work with the data in its original format as long as possible to reduce the latency due to the data transformations.}
...
% \begin{enumerate}[leftmargin=32pt,label=\textsf{Hp.\arabic*}]
% \setcounter{enumi}{1}
% \item A streaming computational model that defers as long as possible the data transformation is less complex, in terms of time and space, than a computational model that cast data into RDF at ingestion time.
% \end{enumerate}

% Moreover, another important aspect in big data analysis problems is \emph{the time at which the analysis is run and the results are produced}. This is a crucial point that combines business requirements and technical constraints associated to the amount of information to process and the computational power availability. 

% In the experiments presented in Chapter~\ref{ch:eval} we distinguish among three different methods for different purposes and stakeholders: (i) \textbf{real-time}, i.e., continuous collection, augmentation and synthesis of data followed by immediate analysis and result display; (ii) \textbf{periodic}, i.e. continuous collection, augmentation and synthesis of data followed by periodic analysis, with arbitrary, but known, temporal lag before result rendering; and (iii) \textbf{a-posteriori}, i.e., ad-hoc analyses performed after the completion of the phenomenon. 

% In general, real-time analysis was preferred in public space installations with public, animated visualization of results, where it is necessary to engage the audience in front of the installation itself (e.g., letting them see the photos they posted on Instagram). Periodic analysis is useful to compare a recent past against a historical collection of data. We used it to allow a user to visually detect patterns, e.g., a security officer that needs to track the movements of large crowds entering or exiting a large venue. The a-posteriori is useful for all those stakeholders that needs to perform analysis without knowing in advance the frequency. This can be useful to plan services for the urban environment or scheduling events, campaigns and commercial offers that suit the need of citizens.

% \section{Proposed Solution}\label{sec:comp-mod-sol}
% In this section, we present our solution starting from the principles, exposed in Section~\ref{sec:comp-mod-sol-principles}, and concluding with the general architecture behind a system that implements the proposed principles (see Section~\ref{sec:comp-mod-sol-p}).
...
C$\langle\mathrm{T}\rangle(\tau)$ denotes a finite bag of data items at time instant $\tau \in \mathrm{Ti}$. 
...
A typical social media use case require the system to join the information in the flowing posts with the static (slowly evolving) information related to the authors of the such posts.
...
Figure~\ref{fig:cm-c} depicts \river{} conceptual plan.
...
The operator O$_E$ joins the information from the two branch using the user\_id as key and produce an enriched Generic Collection of relational table. Table~\ref{tbl:oe_res} presents the results of the join operation.
